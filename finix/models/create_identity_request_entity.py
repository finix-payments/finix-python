...
class CreateIdentityRequestEntity(ModelNormal):
    ...
    def _from_openapi_data(cls, annual_card_volume, business_address, business_name, business_phone, business_tax_id, business_type, default_statement_descriptor, dob, doing_business_as, email, first_name, has_accepted_credit_cards_previously, incorporation_date, last_name, max_transaction_amount, ach_max_transaction_amount, mcc, ownership_type, personal_address, phone, principal_percentage_ownership, tax_id, title, url, *args, **kwargs):  # noqa: E501
        ...
        Args:
            ...
            business_name (str): The merchant's legal business name (max 120 characters).<ul><li>If **INDIVIDUAL_SOLE_PROPRIETORSHIP**, pass the owner's legal first name, last name and middle initial.</li><li>Special characters (e.g., <code>.</code><code>,</code><code>_</code><code>-</code><code>?</code><code>+</code><code>*</code> or <code>/</code>.) are not allowed.</li></ul>
            ...
            ownership_type (str): Avalible values include: <ul><li><strong>PUBLIC</strong> to indicate a publicly-traded company.<li><strong>PRIVATE</strong> for privately-held businesses.  For `business_type` of **GOVERNMENT_AGENCY** and **TAX_EXEMPT_ORGANIZATION** the `ownership_type` should be set to **PUBLIC**.  For `business_type` **INDIVIDUAL_SOLE_PROPRIETORSHIP**, the `ownership_type` should be set to **PRIVATE**.
