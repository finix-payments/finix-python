...
def test_delete_settlement_transfers(config):
    ...
    with pytest.raises(finix.ApiException) as e:
        tmp_client.settlements.remove_transfers_from_settlement(sid,remove_settlement_transfer = request)
...
def test_create_settlement(config):
    ...
    with pytest.raises(finix.ApiException) as e:
        tmp_client.settlements.create(id,create_settlement_request=request)
...
def test_update_settlement(config):
    ...
    response = tmp_client.settlements.update(id,update_settlement_request=request)
    assert response.id[:2] == 'ST'
    assert response.type == 'MERCHANT_REVENUE'
